function ply2idtf(listply,idtffile)
...
%           The 1st column contains the string with the file name of the
%           PLY file. The 2nd column contains the part name (node name)
%           that the PLY file will have in the IDTF/U3D file. The 3rd
%           column contains a 4x4 affine matrix that repositions the geometry
%           of the PLY file in the world coordinate space.
% idtffile : Output IDTF file, from which an U3D file can be compiled.
% 
...
for p = 1:nP,
    ...
    fprintf(fid,'  NODE_NAME "%s"\n',listply{p,2});
    fprintf(fid,'  PARENT_LIST {\n');
    fprintf(fid,'    PARENT_COUNT 1\n');
    fprintf(fid,'    PARENT 0 {\n');
    fprintf(fid,'      PARENT_NAME "<NULL>"\n');
    fprintf(fid,'      PARENT_TM {\n');
    fprintf(fid,'        %f %f %f %f\n',eye(4));
    fprintf(fid,'      }\n');
    fprintf(fid,'    }\n');
    fprintf(fid,'  }\n');
    fprintf(fid,'  RESOURCE_NAME "m%s"\n',listply{p,2});
    fprintf(fid,'}\n');
end

% Model resources
fprintf(fid,'RESOURCE_LIST "MODEL" {\n');
fprintf(fid,'  RESOURCE_COUNT %d\n',nP); % Number of PLY files
for p = 1:nP,

    % Load the PLY file
    ply = plyread(listply{p,1});
    vtx = [ply.vertex.x ply.vertex.y ply.vertex.z];
    rgb = [ply.vertex.red ply.vertex.green ply.vertex.blue];
    fac = cell2mat(ply.face.vertex_index);
    nV = numel(ply.vertex.x);
    nF = numel(ply.face.vertex_index);

    % Apply affine transformation
    vtmp = [vtx ones(nV,1)];
    vtmp = vtmp*listply{p,3}';
    vtx  = vtmp(:,1:3);
    
    % Number of unique colours
    [urgb,ignore,uidx] = unique(rgb,'rows');
    nC = size(urgb,1);

    % Resource instances
    fprintf(fid,'  RESOURCE %d {\n',p-1);
    fprintf(fid,'    RESOURCE_NAME "m%s"\n',listply{p,2});
    fprintf(fid,'    MODEL_TYPE "MESH"\n');
    fprintf(fid,'    MESH {\n');
    fprintf(fid,'      FACE_COUNT %d\n',nF);                % Number of faces
    fprintf(fid,'      MODEL_POSITION_COUNT %d\n',nV);      % Number of vertices
    fprintf(fid,'      MODEL_NORMAL_COUNT 0\n');            % Number of normals
    fprintf(fid,'      MODEL_DIFFUSE_COLOR_COUNT %d\n',nC); % Number of unique colors
    fprintf(fid,'      MODEL_SPECULAR_COLOR_COUNT 0\n');
    fprintf(fid,'      MODEL_TEXTURE_COORD_COUNT 0\n');
    fprintf(fid,'      MODEL_BONE_COUNT 0\n');
    fprintf(fid,'      MODEL_SHADING_COUNT 1\n');
    fprintf(fid,'      MODEL_SHADING_DESCRIPTION_LIST {\n');
    fprintf(fid,'        SHADING_DESCRIPTION 0 {\n');
    fprintf(fid,'          TEXTURE_LAYER_COUNT 0\n');
    fprintf(fid,'          SHADER_ID 0\n');
    fprintf(fid,'        }\n');
    fprintf(fid,'      }\n');
    fprintf(fid,'      MESH_FACE_POSITION_LIST {\n');
    fprintf(fid,'        %d %d %d\n',fac');
    fprintf(fid,'      }\n');
    fprintf(fid,'      MESH_FACE_SHADING_LIST {\n');
    fprintf(fid,'        %d\n',zeros(nF,1));
    fprintf(fid,'      }\n');
    fprintf(fid,'      MESH_FACE_DIFFUSE_COLOR_LIST {\n');
    fprintf(fid,'        %d %d %d\n',uidx(fac+1)'-1);
    fprintf(fid,'      }\n');
    fprintf(fid,'      MODEL_POSITION_LIST {\n');
    fprintf(fid,'        %f %f %f\n',vtx');
    fprintf(fid,'      }\n');
    fprintf(fid,'      MODEL_DIFFUSE_COLOR_LIST {\n');
    fprintf(fid,'        %f %f %f %f\n',[urgb./255 ones(nC,1)]');
    fprintf(fid,'      }\n');
    fprintf(fid,'    }\n');
    fprintf(fid,'  }\n');
...
% Shaders
fprintf(fid,'RESOURCE_LIST "SHADER" {\n');
fprintf(fid,'  RESOURCE_COUNT 1\n');
fprintf(fid,'  RESOURCE 0 {\n');
fprintf(fid,'    RESOURCE_NAME "NoName"\n');
fprintf(fid,'    ATTRIBUTE_USE_VERTEX_COLOR "TRUE"\n');
fprintf(fid,'    SHADER_MATERIAL_NAME "NoName"\n');
fprintf(fid,'    SHADER_ACTIVE_TEXTURE_COUNT 0\n');
fprintf(fid,'  }\n');
fprintf(fid,'}\n');

% Materials
fprintf(fid,'RESOURCE_LIST "MATERIAL" {\n');
fprintf(fid,'  RESOURCE_COUNT 1\n');
fprintf(fid,'  RESOURCE 0 {\n');
fprintf(fid,'    RESOURCE_NAME "NoName"\n');
fprintf(fid,'    MATERIAL_AMBIENT 0.0 0.0 0.0\n');
fprintf(fid,'    MATERIAL_DIFFUSE 1.0 1.0 1.0\n');
fprintf(fid,'    MATERIAL_SPECULAR 0.0 0.0 0.0\n');
fprintf(fid,'    MATERIAL_EMISSIVE 1.0 1.0 1.0\n');
fprintf(fid,'    MATERIAL_REFLECTIVITY 0.0\n');
fprintf(fid,'    MATERIAL_OPACITY 1.0\n');
fprintf(fid,'  }\n');
fprintf(fid,'}\n');

% Shading modifiers
for p = 1:nP,
    fprintf(fid,'MODIFIER "SHADING" {\n');
    fprintf(fid,'  MODIFIER_NAME "%s"\n',listply{p,2});
    fprintf(fid,'  PARAMETERS {\n');
    fprintf(fid,'    SHADER_LIST_COUNT 1\n');
    fprintf(fid,'    SHADER_LIST_LIST {\n');
    fprintf(fid,'      SHADER_LIST 0 {\n');
    fprintf(fid,'        SHADER_COUNT 1\n');
    fprintf(fid,'        SHADER_NAME_LIST {\n');
    fprintf(fid,'          SHADER 0 NAME: "NoName"\n');
    fprintf(fid,'        }\n');
    fprintf(fid,'      }\n');
    fprintf(fid,'    }\n');
    fprintf(fid,'  }\n');
...
% This function is available in the MatlabCentral File Exchange
% under a BSD license:
% http://www.mathworks.com/matlabcentral/fileexchange/47484-plyread-m
% It is included here so as to facilitate use.
